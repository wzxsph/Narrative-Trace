...
## 56. V0.34 Scene Artifact Layer

V0.33 让 `draft_skeleton` 消费 `scene_blueprint.json`，但 scene 仍然直接来自代码里的 demo scene library。V0.34 再拆一层：新增 `scene_artifacts.json`，把每个场景变成可导出、可校验、可替换的中间产物。

这一步的产品意义很直接：未来 agent 不能只生成“完整 game”，它需要能逐场景生成、逐场景审稿、逐场景回滚。
...
```text
generated/<run>/scene_artifacts.json
```
...
- `project_id`
- `source_scene_blueprint_version`
- `entry_scene_id`
- `artifacts`
...
`validate_scene_artifacts` 覆盖：

- artifact schema version。
- artifact entry 必须匹配 blueprint entry。
- artifact scene 顺序必须匹配 blueprint scene 顺序。
- artifact scene id 必须和内嵌 scene.id 一致。
- artifact scene 必须属于 blueprint。
- artifact 必须包含 blueprint 计划的 observe targets、choice targets、state writes。

### 56.4 Draft Skeleton Change

`draft_skeleton` 从消费 `scene_blueprint` 编译 game，改为消费 `scene_artifacts` 编译 game：

```text
scene_blueprint -> scene_artifacts -> game
```

当前 `scene_artifacts` 仍由 demo scene library 生成，但它已经是独立产物。后续可以替换为：

- 单场景 LLM draft artifact。
- 人类审稿后的 locked artifact。
- 多版本 scene artifact。
- 失败 scene 的局部 repair artifact。

### 56.5 Acceptance

V0.34 必须满足：

- `agent_trace.jsonl` 出现 `draft_scene_artifacts` 和 `validate_scene_artifacts`。
- agent 输出目录包含 `scene_artifacts.json`。
- `draft_skeleton` trace 的 `draft_source` 为 `scene_artifacts_v0_1`。
- 独立 CLI 可以校验 `scene_artifacts.json`。
- 单测覆盖 artifact 通过、顺序不匹配失败、缺 planned choice 失败、蓝图引用缺失 library scene 失败。
- 完整离线 agent 仍可通过 state schema、scene blueprint、scene artifacts、blueprint alignment、JSON schema、结构、内容 QA、smoke、save contract 等门禁。
...
## 57. V0.35 Scene Artifact Review and Release Gate

V0.34 让 scene 成为 `scene_artifacts.json`，但 artifact 只有 `draft` 状态，没有 release 语义。工业级创作 agent 不能让“刚生成的草稿”直接进入最终 game，必须有一个明确的审稿/锁定门禁。

V0.35 新增 deterministic review/release gate。它仍不是最终的人类审稿工作台，但已经把状态机跑通：

```text
draft -> reviewed -> locked -> compile
```
...
### 57.2 Review Metadata

每个 scene artifact 增加：
...
```text
review_schema_version = scene_artifact_review_v0_1
```

### 57.3 Release Rules

`validate_scene_artifact_release` 必须保证：

- 存在 `review_schema_version`。
- 每个 artifact 的 `status` 必须是 `locked`。
- 每个 artifact 的 `review.state` 必须是 `approved`。
- 每个 artifact 必须有非空 reviewer。

`compile_game_from_scene_artifacts` 在编译前也会执行 release 校验；未 locked 的 artifact 不能进入最终 game。

### 57.4 CLI

`validate_scene_artifacts.py` 新增：
...
- `agent_trace.jsonl` 出现 `review_scene_artifacts` 和 `validate_scene_artifact_release`。
- 导出的 `scene_artifacts.json` 包含 review schema 和 locked artifacts。
- `draft_skeleton` trace 包含 locked scene artifact 数量。
- `compile_game_from_scene_artifacts` 拒绝未 locked artifacts。
- 独立 CLI 可以校验 scene artifact release 状态。
- 完整离线 agent 仍可通过 state schema、scene blueprint、scene artifact、release、blueprint alignment、JSON schema、结构、内容 QA、smoke、save contract 等门禁。
...
## 59. V0.37 LLM Scene Review Probe

V0.36 证明 `.env` 真实 LLM 调用和 `optional_llm_polish` 路径可用，但 LLM 仍只参与第一段文本 polish。V0.37 新增受控的 LLM scene review probe：让模型审查一个 scene artifact，并返回结构化 review JSON。

这一步仍然不让 LLM 修改 game，也不让 LLM 接管 release。它的意义是把“LLM 可参与审稿”纳入 agent graph，但维持 deterministic gates 的最终控制权。
...
- `provider=offline`：跳过。
- `provider=auto`：有 `.env` 时尝试，失败则 fallback。
- `provider=llm`：必须成功，否则 graph 失败。
...
LLM scene review 是 probe，不是 release gate：

- 不修改 scene artifact。
- 不锁定 artifact。
- 不改变 game。
- 不替代 deterministic `validate_scene_artifacts` 和 `validate_scene_artifact_release`。

### 59.5 Acceptance

V0.37 必须满足：

- `agent_trace.jsonl` 在 offline 下出现 `optional_llm_scene_review` 且状态为 `skipped`。
- `provider=llm` 路径实际调用 `.env` 并生成 `llm_scene_review.json`。
- 独立 CLI `scripts/llm_scene_review_smoke.py` 可真实调用 `.env` 并通过。
- 单测覆盖 LLM review JSON normalization。
- 完整 offline 验证矩阵和真实 LLM smoke/agent smoke 均通过。
...
## 60. V0.38 Review Issues Artifact

V0.37 让 LLM 可以审查 scene artifact，但 `verdict=revise` 的结果只存在于 `llm_scene_review.json` 和 trace 中，尚未变成可追踪的工作项。V0.38 新增 `review_issues.json`，把 LLM 审稿意见沉淀为非阻断 issue artifact。

这一步的产品意义是把“模型意见”从一次性输出变成团队工作流：

- 可以汇总。
- 可以分配给人类审稿。
- 可以标记 resolved。
- 可以未来进入 release policy。
...
```text
generated/<run>/review_issues.json
```
...
- LLM review `verdict=pass` 不生成 issue。
- LLM review `verdict=revise` 将每个非 `none` risk flag 转换为一个 open issue。
- 当前 issue 均为 `blocking=false`，不会阻断 deterministic release。
- `major/minor/info` severity 由 risk flag 映射。
...
- offline 输出 `review_issues.json`，且 issues 为空。
- `provider=llm` 若 LLM review 返回 `revise`，输出非空 `review_issues.json`。
- review issue artifact 有独立 validator 和 CLI。
- 单测覆盖 revise 生成 issue、pass 为空、非法 issue 失败。
- 完整 offline 验证矩阵和真实 LLM smoke/agent smoke 均通过。

...
- 场景数量少于完整 V0 推荐规模。
- 章节复盘已出现，但目前还是轻量复盘屏，不是完整 Detroit 式章节 flowchart。
- 隐藏状态已经影响 choice、章节复盘和结局画像，但 NPC 关系的长线文本回声仍较轻。
...
1. 扩展 demo 到至少 3 章、每章 3 个主场景，接近 15 到 20 分钟体验。
2. 增加第一章轻教学，明确“文字可以展开”和“观察会解锁行动”。
3. 把章节复盘从“轻量列表”升级为更接近 flowchart 的节点关系复盘。
4. 增强 NPC 关系回声：关键关系变量至少在 2 个后续节点有文本反馈。
...
```text
带基础运行时韧性的可运行工程纵切 / playable vertical slice with basic runtime resilience
```

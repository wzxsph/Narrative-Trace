...
- 章节复盘已从轻量列表升级为基础 flowchart，但还不是完整 Detroit 式全路径图，缺少节点坐标、锁定分支、二周目提示和跨章路径关系。
...
2. 建立内部测试记录模板，用用户反馈验证第 14 节成功标准。
3. 检查 9 场景内容节奏，删掉只是在“加长度”的场景，保留真正改变理解或行动的场景。
4. 继续打磨关系回声文案，确保它是文本暗示，不是数值播报。
5. 将章节基础 flowchart 继续升级为更完整的路径复盘，包括未解锁分支原因与跨章影响。

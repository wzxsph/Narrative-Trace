...
1. 增加保存/恢复进度能力。
2. 增加章节结束复盘屏，而不是只提供路径图侧栏。
3. 将 `repair_game.py` 从报告工具升级为可修复 broken anchor / missing choice / missing next_scene 的局部 repair。
4. 增加 `tests/`，覆盖 validator、runtime playthrough、generated artifact shape。
5. 增加 `README.md`，让新 agent 或人类可以一键运行 demo。

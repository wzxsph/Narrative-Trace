...
- 浏览器级移动视口 smoke：验证轻教学、新行动高亮、章节复盘、未解锁原因、刷新恢复、坏存档 fallback、恢复提示和移动端横向溢出。
...
- 保存/恢复已支持单机本地状态，并已有浏览器 smoke 覆盖章节复盘刷新恢复、坏 JSON 存档 fallback、旧版本存档 fallback 和恢复提示；但还没有多存档、云端同步或版本迁移。

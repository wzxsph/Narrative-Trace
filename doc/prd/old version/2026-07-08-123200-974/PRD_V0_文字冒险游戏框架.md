...
V0.21 相比 V0.20 的实质变化：

- 旧版 V0.20 PRD 和技术文档已归档到 `/home/samsong/Desktop/game_writer/doc/prd/old version/2026-07-08-115126-273`。
- `src/app.js` 新增 `runtime.recoveryNotice` 和 `renderRecoveryNotice()`，坏存档 fallback 后在故事正文上方显示“进度已重置”提示。
- 坏 JSON 存档提示文案为“旧进度内容损坏，已为你开启新局。”
- 旧版本/不兼容存档提示文案为“旧进度与当前案件不兼容，已为你开启新局。”
- 玩家继续进行 observe 或 choice 后，恢复提示会自动消失，避免长期占用故事界面。
- `scripts/browser_smoke.py` 不再只检查 fallback 到首场景，还检查恢复提示存在且文案正确。
- `tests/test_demo_contract.py` 增加恢复提示静态 hook。
- `src/styles.css` 增加 `.recovery-notice` 样式。

V0.21 的边界：

- 当前提示只说明旧进度不可恢复并开启新局，不提供恢复旧存档、导出坏存档或提交错误报告。
- 当前仍没有多存档、存档版本迁移、云同步或损坏存档备份。
- 下一步如继续做存档系统，应先定义版本迁移策略和存档数据最小合同。

### 17.26 V0.22 版本变更记录

V0.22 相比 V0.21 的实质变化：

- 旧版 V0.21 PRD 和技术文档已归档到 `/home/samsong/Desktop/game_writer/doc/prd/old version/2026-07-08-115913-183`。
- `src/app.js` 将 `SAVE_VERSION` 从 1 升级到 2。
- `src/app.js` 新增 `migrateSavePayload()`，当前支持 v1 存档迁移到 v2。
- `restoreProgress()` 先迁移再校验，避免已有 v1 本地进度因版本号变化直接被判坏。
- `scripts/browser_smoke.py` 增加 v1 存档迁移检查：把章节复盘存档降级为 v1 后刷新，必须恢复复盘页。
- `tests/test_demo_contract.py` 增加 `SAVE_VERSION = 2` 和 `migrateSavePayload` 静态 hook。
- README 同步 v1 存档迁移覆盖范围。
